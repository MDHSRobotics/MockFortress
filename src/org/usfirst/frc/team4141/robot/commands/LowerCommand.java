...
public class LowerCommand extends MDCommand {
	...
	private ScissorLiftSubsystem scissorLiftSubsystem;
	...
	public LowerCommand(MDRobotBase robot, String name) {
		super(robot, name);
		if(!getRobot().getSubsystems().containsKey("scissorLiftSubsystem")){
			log(Level.ERROR, "initialize()", "Scissor Lift subsystem not found");
			throw new IllegalArgumentException("Scissor Lift Subsystem not found");
		}
		scissorLiftSubsystem = (ScissorLiftSubsystem)getRobot().getSubsystems().get("scissorLiftSubsystem"); 
		requires(scissorLiftSubsystem);
	}
	...
	protected void execute() {
		if (scissorLiftSubsystem!=null)scissorLiftSubsystem.lower();
